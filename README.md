...
| `claude-oauth-token` | долгоживущий токен Claude (`claude setup-token`) — основной путь |
| `claude-credentials.json` | копия сессионного токена — запасной путь, живёт до первой ротации |
...
Нужен **долгоживущий токен**. Делается один раз, в любом залогиненном контейнере:

```bash
ssh omv-dev-myproj
claude setup-token          # выдаст токен вида sk-ant-oat01-…
exit

# положить на хост (ввод с клавиатуры, Ctrl-D в конце):
ssh omv 'umask 077; cat > /root/claude-dev-build/claude-oauth-token'
```

Дальше каждый новый контейнер получает его при `deploy`, существующие — через
`retrofit --force`. Токен кладётся в `~/.claude_env` (0600) и экспортируется как
`CLAUDE_CODE_OAUTH_TOKEN` из `/etc/profile.d` и `~/.bashrc`.

### Почему не копированием `.credentials.json`

Так кит делал раньше, и это тихо ломается. Refresh-токены **ротируются**: контейнер,
обновивший свой токен первым, получает новый refresh-токен, а копия во всех остальных
контейнерах — и в мастере на хосте — в тот же момент становится недействительной. Пока
скопированный access-токен жив (несколько часов), всё выглядит рабочим; при первой же
попытке обновиться контейнер уходит на экран входа. Со стороны это неотличимо от «креды
не сработали», хотя файл на месте и не просрочен.

Копирование осталось запасным путём — если долгоживущего токена нет, `deploy` кладёт
`.credentials.json` и честно предупреждает, сколько это проживёт.

Отдельная тонкость: логин — это **два** файла. Токен в `~/.claude/.credentials.json` и
опознание аккаунта в `~/.claude.json` (`oauthAccount`, `userID`, онбординг). Без второго
Claude открывает вход даже с валидным токеном. `save-creds` снимает обе половины разом;
опознание подмешивается, не трогая остального — `machineID` и доверенные каталоги там
про конкретную машину.

`doctor` не разглядывает файлы, а спрашивает `claude auth status`: файл на месте и
логин работает — это разные утверждения, и путать их дорого.
